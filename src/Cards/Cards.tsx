...
const Cards = () => {
  ...
  const { paintings: cards } = useAppSelector(
  ...
  return (
    <div className={classes.cards}>
      {cards.length === 0 ? (
        <div
          className={`${classes.cards__results} ${
            darkMode ? classes.cards__results_dark : ""
          }`}
        >
          No results{" "}
        </div>
        ...
        cards?.map((painting) => (
          <CardItem
            ...
            author={
              authors &&
              authors.find((author) => author.id === painting.authorId)
            }
            location={
              locations &&
